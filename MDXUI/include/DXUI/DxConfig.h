...
#define ulong unsigned long
	...
	//
	//+----------------------------
	static void DxColor2HSL(const DxColor& clr, double& pHue, double& pSat, double& pLue){
		float R = (float)(clr.rgb.r) / 255.0f; 
		float G = (float)(clr.rgb.g) / 255.0f;
		float B = (float)(clr.rgb.b) / 255.0f;
	...
	//
	//+-----------------------------
	static DxColor HSL2DxColor(double H, double S, double L){
		BYTE R, G, B;
		float var_1, var_2;
		...
		else
		{
			...
			R = (BYTE)(255.0f * Hue2RGB(var_1, var_2, H + (1.0f / 3.0f)));
			G = (BYTE)(255.0f * Hue2RGB(var_1, var_2, H));
			B = (BYTE)(255.0f * Hue2RGB(var_1, var_2, H - (1.0f / 3.0f)));
		}

		unsigned long val = RGB(R, G, B);
		return RgbI(GetRValue(val), GetGValue(val), GetBValue(val));
	}
